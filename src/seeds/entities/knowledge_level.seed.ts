...
import { KnowledgeLevel } from '../../entities/knowledge_level.entity';  // Assurez-vous du bon chemin
...
export class KnowledgeLevelSeeder extends BaseSeeder {
  ...
  async seed(): Promise<void> {
    ...
    const knowledgeLevels = [
      { rating: 1, name: 'Débute', description: "Connaissances de base, a besoin d'être accompagné" },
      { rating: 2, name: 'Occupe', description: 'Connaissances essentielles, autonomie partielle' },
      { rating: 3, name: 'Maitrise', description: 'Connaissances confirmées, autonomie sur le poste' },
      { rating: 4, name: 'Référent', description: 'Reconnu par ses collègues et sollicité pour des conseils' },
    ];
    ...
    if (countInDb === knowledgeLevels.length) {
      ...
      const allMatch = knowledgeLevels.every(level =>
        existingLevels.some(existingLevel =>
